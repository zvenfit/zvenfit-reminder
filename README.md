...
Основной режим — универсальный диспетчер обязательств для нескольких Telegram-
групп. Он поддерживает разовые и повторяющиеся напоминания, ответственность,
...
| `ALLOWED_CHAT_ID` | ID прежней основной группы; нужен только для совместимости legacy-режима |
...
`UNIVERSAL_REMINDERS_ENABLED=1`. Production-пайплайн перед обновлением функций
применяет только ещё не выполненные YDB-миграции и проверяет их контрольные
суммы.
...
- `/sync` — повторно проверить уже известных боту участников
